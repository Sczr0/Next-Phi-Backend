...
> | 遗留 D1（state.db/stats.db 拆分） | ✅ **已实施**（ADR-0002：双池 + DDL 分组 + `db_split` 迁移工具（表集互斥断言） + 配置 `stats.state_db_path` | 本轮 |
> | 遗留 D5（save_submissions 保留策略） | ✅ **已启用**（owner 拍板 100；config.example `save_submissions_retention_per_user = 100`；ADR-0003 已接受） | 本轮 |
>
