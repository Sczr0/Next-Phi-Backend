...
// ── Phase 2 端口收口：领域行结构体已物化到 phi-contract/storage.rs ──
// （端口 trait 的返回值类型必须位于契约层）。此处 re-export 保持
// `impl_storage::storage::LeaderboardTopRow` 等路径不变。
pub use phi_contract::storage::{
