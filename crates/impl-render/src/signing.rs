...
/// 底栏 `<text>` 元素上的标记属性，注入时靠它定位底栏。
/// 不依赖文案内容：`branding.footer_text` 为自定义文案时定位不受影响
/// （旧实现搜索 "Generated by Phi-Backend" 子串，自定义文案会让签名行
/// 误入画布居中的 fallback 分支）。
const FOOTER_MARKER: &str = "data-lilith-footer=\"1\"";

/// 签名行 `<text>` 元素上的标记属性，用于注入后识别与剥离。
const SIG_FOOTER_MARKER: &str = "data-lilith-sig=\"1\"";

/// 签名行专用样式类：与底栏同字号/颜色，但**不声明 `text-anchor`**——
/// 锚点一律由注入逻辑显式写在元素属性上。
/// 不得复用 `.text-footer`：单曲图样式表里它声明 `text-anchor: end`，
/// 按 SVG 级联规则 CSS 规则优先于 presentation attribute，会把元素上的
/// `text-anchor="middle"` 覆盖成 `end`，签名行被锚在 x=50% 处向左伸出画布，
/// 表现为底部文字错位（已回归确认）。
const SIG_LINE_CLASS: &str = "text-sig-line";
...
/// 将签名字符串拆为 2~3 行，适配宽度约 1120px。
/// v4 在 `:t=` 和 `:root=` 处拆为三行（sig→第一行, t+uid+rid→第二行, root+n+nonce→第三行）；
/// v3 在 `:hash=` 处拆为两行。line[0..N-1] 末尾保留 `:`，line[N-1] 不含尾 `:`。
fn wrap_sig_lines(full: &str) -> Vec<String> {
    // v4 三行拆分（优先）
    if let (Some(i1), Some(i2)) = (full.find(":t="), full.rfind(":root="))
        && i1 < i2
    {
        let mid = &full[i1 + 1..i2]; // 不含开头 ':'，不含 ':root='
        return vec![
            format!("{}:", &full[..i1]),
            format!("{}:", mid),
            full[i2 + 1..].to_string(),
...
/// 将签名行注入到 SVG 底栏：在现有底栏文本下方换行，复用其 `x` /
/// `text-anchor`（均为元素属性），样式用独立的 `text-sig-line` 类。
///
/// 签名串以 `:` 拆为两行（`<tspan>`）：
...
///
/// 若 SVG 中找不到底栏标记（例如单测手写的最小 SVG），则退化为在画布底部
/// 居中渲染签名行。
///
...
pub fn inject_sig_footer(svg: &str, sig: &SvgSignature) -> String {
    ...
    let text_elem = match locate_footer_tag(svg) {
...
/// 构造紧接底栏正文下方的签名 `<text>` 元素，复用底栏的 `x` / `text-anchor`
/// 属性，样式固定为 `text-sig-line`（不继承底栏 class，避免底栏 CSS 里的
/// `text-anchor` 声明覆盖本元素的锚点属性）；`y` 在原行 baseline 上加一个
/// 行高；后续每行用 `<tspan x=… dy=行高>` 接续。
fn build_sig_text_below(tag: &str, lines: &[String]) -> String {
    ...
    let mut out = format!(
        "<text x=\"{x}\" y=\"{y0:.1}\" class=\"{SIG_LINE_CLASS}\"{anchor_attr} {SIG_FOOTER_MARKER}>"
    );
...
/// 退化路径：找不到底栏标记时，在画布底部居中渲染签名行（多行同样处理）。
fn build_sig_text_fallback(svg: &str, lines: &[String]) -> String {
    ...
    let x = "50%";
    let mut out = format!(
        "<text x=\"50%\" y=\"{y0:.1}\" class=\"{SIG_LINE_CLASS}\" text-anchor=\"middle\" {SIG_FOOTER_MARKER}>"
    );
    for (i, line) in lines.iter().enumerate() {
        if i == 0 {
            let _ = write!(out, "<tspan x=\"{x}\">{line}</tspan>");
...
/// 在 SVG 中定位带 `data-lilith-footer="1"` 标记的底栏 `<text ...>` 开标签子串。
/// 返回从 `<text ` 到该开标签闭 `>` 为止的片段（含两端）。
fn locate_footer_tag(svg: &str) -> Option<&str> {
    let text_pos = svg.find(FOOTER_MARKER)?;
    // 从标记向左找最近的 <text 开标签起始。
...
<text class="lilith-sig-footer">lilith-sig:v3:hmac=abcdef1234567890:t=1721029907:uid=test1234:hash=sha256abc:nonce=01900000-0000-7000-8000-000000000001</text>
  ...
  <text x="10" y="20">Best 30</text>
    ...
    // ── 回归：有底栏正文时，签名行应换行接在其下方、复用其 x / 锚点属性 ──

    #[test]
    fn test_sig_footer_below_generated_text() {
        let cfg = test_signing_config();
        // 模拟真实 BestN 渲染器输出：高画布 + 带引号 viewBox + 底栏正文行（居中）。
        // footer_height=122，垂直布局 → generated_text y = total - 74。
        let total_h = 1480.0_f64;
        let gen_y = total_h - 74.0; // 1406
        let svg = format!(
            r#"<svg width="820" height="1480" viewBox="0 0 820 1480" xmlns="http://www.w3.org/2000/svg">
  <text x="50%" y="{gen_y}" class="text-footer" text-anchor="middle" data-lilith-footer="1">Generated by Phi-Backend at 2026/06/28 03:42:21 UTC+8</text>
</svg>"#
        );
        ...
        // 签名 <text> 用独立 sig 类（不继承底栏 class）、复用 x / text-anchor（middle），
        // 并把签名拆为两行 tspan。
        ...
        assert!(injected_elem.contains("class=\"text-sig-line\""));
        assert!(!injected_elem.contains("class=\"text-footer\""));
        assert!(injected_elem.contains("text-anchor=\"middle\""));
        assert!(injected_elem.contains("x=\"50%\""));
    ...
    // ── 回归：底栏定位只认 marker 属性，自定义 footer 文案不得改变注入分支 ──

    #[test]
    fn test_sig_footer_locates_by_marker_even_with_custom_text() {
        let cfg = test_signing_config();
        // 生产事故场景：branding.footer_text 为自定义文案，底栏不含
        // "Generated by Phi-Backend" 子串。旧实现（搜文案）会误入画布底部
        // 居中 fallback，且 .text-footer 的 CSS text-anchor:end 覆盖
        // middle 属性，签名行被锚在 x=50% 向左伸出画布。marker 定位下：
        // 签名行必须接在底栏下方并继承其 end 锚点。
        let svg = r#"<svg width="1400" height="840" viewBox="0 0 1400 840" xmlns="http://www.w3.org/2000/svg">
  <text x="1360" y="784" class="text-footer" text-anchor="end" data-lilith-footer="1">由 example.com 支持</text>
</svg>"#;
        ...
        let sig_y = extract_injected_y(&signed).expect("injected y");
        // 接在底栏（y=784）下方一行，而非 fallback 的画布底部（840-30=810）。
        approx_eq(sig_y, 784.0 + FOOTER_LINE_HEIGHT);
        assert!(signed.contains("class=\"text-sig-line\""));
        assert!(signed.contains("text-anchor=\"end\""));
        // 往返仍正确。
        assert_eq!(strip_svg_signature(&signed), svg);
        verify_svg_signature(&signed, &cfg).expect("verify");
    }
