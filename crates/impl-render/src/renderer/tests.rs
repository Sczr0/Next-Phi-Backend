...
// 回归：单曲图底部三段式——声明行居中、底栏带 marker、画布 840。
#[test]
fn generate_song_svg_contains_disclaimer_and_footer_marker() {
    ensure_config_inited();
    let data = SongRenderData {
        song_name: "RemoteSong".to_string(),
        song_id: "SONG REMOTE 456".to_string(),
        ...
        difficulty_scores: std::collections::HashMap::default(),
        illustration_path: None,
        custom_footer_text: Some("Powered by example.com".to_string()),
        disclaimer_text: Some("本项目为非官方玩家项目测试声明。".to_string()),
    };
    ...
    // 自定义 footer 文案存在时不再包含默认 Generated by 文案。
    assert!(!svg.contains("Generated by Phi-Backend"));
}

// 回归：B27 底栏同样带声明行与 marker。
#[test]
fn generate_bn_svg_contains_disclaimer_and_footer_marker() {
    ensure_config_inited();
    let record = RenderRecord {
        song_id: "DISCLAIMER_TEST".to_string(),
        song_name: "Disclaimer".to_string(),
        difficulty: "IN".to_string(),
        score: Some(1_000_000.0),
        acc: 100.0,
        rks: 15.0,
    ...
    .unwrap();
    assert!(svg.contains(r#"class="text-disclaimer" text-anchor="middle""#));
    assert!(svg.contains("本项目为非官方玩家项目测试声明。"));
    assert!(svg.contains(r#"data-lilith-footer="1""#));
    // 声明行在生成行上方一行（18px）。
    let attr_y_before = |marker: &str| {
        svg.split(marker)
            .next()
            .and_then(|seg| seg.rsplit("y=\"").next())
            .and_then(|v| v.split('"').next())
            .and_then(|v| v.parse::<f64>().ok())
            .expect("y attr before marker")
    };
    let disclaimer_y = attr_y_before(r#"class="text-disclaimer""#);
    let footer_y = attr_y_before(r#"data-lilith-footer="1""#);
    assert!((footer_y - disclaimer_y - 18.0).abs() < 1e-6);
}
