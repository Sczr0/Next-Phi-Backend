...
use phi_contract::song::SongCandidatePreview;
...
///
/// `pub`（原 `pub(crate)`）：跨 crate 后由根 crate 的 `src/error.rs` shim
/// (`pub use phi_http::error::*`) 保持 `crate::error::sanitize_reqwest_error`
/// 可达（features/auth、features/open_platform 使用）。
#[must_use]
pub fn sanitize_reqwest_error(err: &reqwest::Error) -> String {
